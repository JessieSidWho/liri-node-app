...
// Inquirer
var infoBot = function(){inquirer.prompt({
  
      name: "category",
      type: "list",
      message: "What type of information would you like to search?",
      choices: ["Movie Info", "Concert Info", "Song Info"]
  
  }).then(function(response){

    var done = true;
  
    if(response.category === "Movie Info"){
      inquirer.prompt({
          name: "answer",
          type: "input",
          message: "What's the name of the Movie you are interested in?",
      }).then(function(response){
        if (response.answer === ""){
          console.log(`If you haven't watched "Mr. Nobody," then you should: <http://www.imdb.com/title/tt0485947/> It's on Netflix!`);
        //   infoBot();
        } else {
          process.argv[3] = response.answer;
          runMovie();
        }
      })
    ...
    if(response.category === "Concert Info"){
      inquirer.prompt({
          name: "answer",
          type: "input",
          message: "What's the name of the Artist whose Concert you are interested in?",
      }).then(function(response){
        if(response.answer === ""){
          console.log("Seems like you didn't request any information.");
        } else {
          process.argv[3] = response.answer;
          runBands();
        }
    ...
    if(response.category === "Song Info"){
      inquirer.prompt({
          name: "answer",
          type: "input",
          message: "What's the name of the Song you are interested in?",
      }).then(function(response){
        if(response.answer === ""){
          defaultSpotify();
        } else {
